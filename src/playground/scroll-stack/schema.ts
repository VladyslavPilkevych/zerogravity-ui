...
export const SCROLL_STACK_CONTROLS: ControlGroup[] = [
    {
        ...
        open: true,
        controls: [
            { kind: "cssLength", path: "height", label: "Section height", min: 40, max: 120, step: 5, unit: "vh" },
            {
                kind: "select",
                path: "sizeMix",
            ...
            { kind: "number", path: "top", label: "Sticky offset", min: 0, max: 200, step: 4, unit: "px" },
            { kind: "number", path: "peek", label: "Peek per card", min: 0, max: 80, step: 2, unit: "px" },
            { kind: "number", path: "rounded", label: "Corner radius", min: 0, max: 48, step: 2, unit: "px" },
            { kind: "number", path: "cards", label: "Sections (demo only)", min: 2, max: 8, step: 1 },
        ],
    },
    {
        id: "motion",
        ...
        open: true,
        controls: [
            { kind: "number", path: "scaleTo", label: "Scale down to", min: 0.6, max: 1, step: 0.01 },
            { kind: "number", path: "dim", label: "Dim overlay", min: 0, max: 1, step: 0.01 },
            { kind: "color", path: "dimColor", label: "Dim colour" },
            { kind: "number", path: "opacityTo", label: "Fade to (see-through)", min: 0, max: 1, step: 0.01 },
            { kind: "number", path: "liftTo", label: "Lift up by", min: 0, max: 160, step: 4, unit: "px" },
            { kind: "number", path: "blurTo", label: "Blur", min: 0, max: 20, step: 0.5, unit: "px" },
...
export const SCROLL_STACK_PRESET_VALUES: Record<string, Partial<ScrollStackDemoConfig>> = {
    ...
    cinematic: { scaleTo: 0.84, dim: 0.68, blurTo: 7, rounded: 32, top: 18, peek: 8, height: "92vh" },
}
