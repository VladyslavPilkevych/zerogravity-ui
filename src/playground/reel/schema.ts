...
export const REEL_CONTROLS: ControlGroup[] = [
    {
        ...
        open: true,
        controls: [
            { kind: "number", path: "itemWidth", label: "Item width", min: 120, max: 640, step: 10, unit: "px" },
            { kind: "number", path: "itemHeight", label: "Item height", min: 160, max: 720, step: 10, unit: "px" },
            { kind: "number", path: "spacing", label: "Spacing", min: 60, max: 700, step: 5, unit: "px" },
            { kind: "number", path: "visible", label: "Neighbours each side", min: 1, max: 8, step: 1 },
    ...
    },
    {
        ...
        controls: [
            ...
            { kind: "number", path: "rotate", label: "Y rotation", min: 0, max: 70, step: 1, unit: "°" },
            { kind: "number", path: "depth", label: "Push back", min: 0, max: 500, step: 10, unit: "px" },
            { kind: "number", path: "perspective", label: "Perspective", min: 400, max: 3000, step: 50, unit: "px" },
        ],
    },
    {
        id: "input",
        ...
        open: true,
        controls: [
            { kind: "number", path: "stiffness", label: "Spring stiffness", min: 2, max: 24, step: 0.5 },
...
export const REEL_PRESET_VALUES: Record<string, Partial<ReelDemoConfig>> = {
    flat: {},
    coverflow: { rotate: 42, depth: 180, spacing: 230, scale: 0.88, opacity: 0.55, perspective: 1100 },
    tight: { spacing: 150, scale: 0.6, opacity: 0.22, visible: 6, itemWidth: 240, itemHeight: 340, loop: true },
    single: { itemWidth: 460, itemHeight: 300, spacing: 520, scale: 0.72, opacity: 0.16, stiffness: 6 },
}
